...
// the <xvm_tlm_fifo_base #(T)> super class, and the interface methods provided by
// these exports are defined by the <xvm_tlm_if_base #(T1,T2)> class.
//------------------------------------------------------------------------------

class xvm_tlm_fifo #(type T=int) extends xvm_tlm_fifo_base #(T);

  `uvm_component_param_utils(xvm_tlm_fifo#(T))
  `uvm_type_name_decl("xvm_tlm_fifo #(T)")
 ...
 `endif 
  ...
  // The ~parent~ should be ~null~ if the <xvm_tlm_fifo#(T)> is going to be used in a
...
//------------------------------------------------------------------------------
//
// Class -- xvm_tlm_analysis_fifo#(T)
//
// An analysis_fifo is a <xvm_tlm_fifo#(T)> with an unbounded size and a write interface.
// It can be used any place a <xvm_analysis_imp> is used. Typical usage is
// as a buffer between a <xvm_analysis_port> in an initiator component
// and TLM1 target component.
//
//------------------------------------------------------------------------------

class xvm_tlm_analysis_fifo #(type T = int) extends xvm_tlm_fifo #(T);
  `uvm_component_param_utils(xvm_tlm_analysis_fifo#(T))
  `uvm_type_name_decl("xvm_tlm_analysis_fifo #(T)")
  ...
  // to an analysis FIFO. 
  // See write method of <xvm_tlm_if_base #(T1,T2)> for more information.

  xvm_analysis_imp #(T, xvm_tlm_analysis_fifo #(T)) analysis_export;
